...
public interface ProductRepository extends Neo4jRepository<Products,Long> {
    ...
    @Query("Match (p1:Products),(p2:Products) where NOT id(p1)=id(p2) AND id(p1)={productId} AND p1.productBrandName=p2.productBrandName AND p1.productSubCategory=p2.productSubCategory create (p1)-[r:SameBrand]->(p2)")
    void postProductSameBrand(@Param("productId") Long productId);

    @Query("Match (p1:Products),(p2:Products) where NOT id(p1)=id(p2) AND id(p1)={productId} AND p1.productBrandName=p2.productBrandName AND p1.productSubCategory=p2.productSubCategory create (p1)-[r:SameBrand]->(p2)")
    void postProductSameBrand2(@Param("productId") Long productId);

    @Query("Match (p1:Products),(p2:Products) where NOT id(p1)=id(p2) AND id(p1)={productId} AND p1.productBrandName=p2.productBrandName AND NOT p1.productSubCategory=p2.productSubCategory AND p1.productCategory=p2.productCategory create (p1)-[r:RelatedProduct]->(p2)")
    void postProductSimilarRelation(@Param("productId") Long productId);

    @Query("Match (p1:Products),(p2:Products) where NOT id(p1)=id(p2) AND id(p1)={productId} AND p1.productBrandName=p2.productBrandName AND NOT p1.productSubCategory=p2.productSubCategory AND p1.productCategory=p2.productCategory create (p1)-[r:RelatedProduct]->(p2)")
    void postProductSimilarRelation2(@Param("productId") Long productId);
